...
| `MAX_CONCURRENT_REQUESTS` | `10` | Sets `--max-running-requests`; also sets the minimum Mamba pool to concurrency × 4 slots |
| `MAX_MAMBA_CACHE_SIZE` | computed minimum | Optional larger Mamba checkpoint pool for long-context branch-prefix retention without increasing request admission. Must be at least `MAX_CONCURRENT_REQUESTS × (4 - MAMBA_SKIP_DECODE_LOCK)` |
...
- **GDN state pool (throughput and prefix retention):** the mandatory minimum for `--max-mamba-cache-size` is concurrency × S; S=4 for `extra_buffer_lazy` + overlap scheduler (no accuracy cost; verified in this build's `kv_cache_configurator.py` — the verify window is a separate buffer, so folding draft tokens in (×8) over-provisions the mandatory per-request pool 2×). `MAX_MAMBA_CACHE_SIZE` can explicitly reserve more checkpoints for long-context branch reuse while `--max-running-requests` continues to pin admission. The TURBO Fable four-slot wrapper uses 64 checkpoints for this reason. `MAMBA_SKIP_DECODE_LOCK=1` drops the mandatory S to 3. The stock `--mamba-full-memory-ratio 0.9` over-provisions KV and clamps concurrency; pinned at 4.21 instead.
